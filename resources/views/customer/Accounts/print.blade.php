    ...
    <div class="invoice-print p-3">
        <div class="invoice-header d-flex justify-content-between flex-md-row flex-column pb-2">
            ...
            <div class="mt-md-0 mt-2">
                <h4 class="font-weight-bold text-right mb-1">{{ __('locale.labels.invoice') }} #{{ $invoice->invoice_number }}</h4>
                ...
                <table>
                    <tbody>
                        <tr>
                            ...
                            {{-- <td><strong>{{ \App\Library\Tool::format_price($invoice->amount, $invoice->currency->format) }}</strong> --}}
                            <td><strong>{{ \App\Library\Tool::format_price($totalAmount, $invoice->currency->format) }}</strong>
                            </td>
                        </tr>
                        <tr>
            ...
            <table class="table m-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ \App\Library\Tool::format_price($totalPlanPrice, $invoice->currency->format) }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
                ...
                <div class="invoice-total-wrapper">
                    ...
                    @if ($invoice->amount > 0)
                        <div class="invoice-total-item">
                            ...
                            <p class="invoice-total-amount">
                                ...
                                {{ \App\Library\Tool::format_price($totalPlanPrice, $invoice->currency->format) }}</p>
                        </div>
                    @endif
                    <div class="invoice-total-item">
                        <p class="invoice-total-title">Addon Amount:</p>
                        <p class="invoice-total-amount">
                            {{-- {{ \App\Library\Tool::format_price($invoice->amount, $invoice->currency->format) }}</p> --}}
                            {{ \App\Library\Tool::format_price($totalAddonsPrice, $invoice->currency->format) }}</p>
                    </div>
                    @if ($totalAdjustedAmount > 0)
                        <div class="invoice-total-item">
                            ...
                            <p class="invoice-total-amount">
                                {{-- {{ Tool::format_price($invoice->amount, $invoice->currency->format) }}</p> --}}
                                {{ \App\Library\Tool::format_price($totalAdjustedAmount, $invoice->currency->format) }}</p>
                        </div>
                    @endif
                    <hr class="my-50" />
                    <div class="invoice-total-item">
                        ...
                        <p class="invoice-total-amount">
                            {{-- {{ \App\Library\Tool::format_price($invoice->amount, $invoice->currency->format) }}</p> --}}
                            {{ \App\Library\Tool::format_price($totalAmount, $invoice->currency->format) }}</p>
                    </div>
                </div>
            </div>
        </div>
