    ...
    <div class="card-body my-2 py-25">
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="mb-2 pb-50">
                    <h5>{!! __('locale.subscription.active_until', [
                        'date' => Tool::formatDateTime($subscription->current_period_ends_at),
                    ]) !!}</h5>
                    <span>{{ __('locale.subscription.current_plan_notification') }}</span>
                </div>
                <div class="mb-1">
                    <h5>{{ Tool::format_price($plan->price, $plan->currency->format) }} {{ __('locale.labels.per') }}
                ...
                <div class="row">
                    <div class="col-md-6">
                        <button class="btn btn-warning waves-effect" type="button" disabled="">
                            ...
                            <span
                                class="text-uppercase">{{ __('locale.subscription.payment_is_being_verified') }}</span>
                        </button>
                    </div>
                </div>
            @else
                <div class="col-12 subscriptionbutton">
                    @if (!$isPlanExpired)
                        <a href="{{ route('customer.subscriptions.renew', $subscription->uid) }}"
                            class="btn btn-primary me-1 mt-1"><i data-feather="repeat"></i>
                            {{ __('locale.labels.renew') }}</a>
                        <a href="{{ route('customer.subscriptions.change_plan', $subscription->uid) }}"
                            class="btn btn-success me-1 mt-1"><i data-feather="refresh-cw"></i>
                            {{ __('locale.labels.change_plan') }}</a>
                        <a href="{{ route('customer.subscriptions.addons', Auth::user()->customer?->subscription?->uid) }}"
                            id="addmorebtn"
                            class="btn btn-subaddon me-1 mt-1"><i data-feather="stop-circle"></i> Buy
                            Add-On</a>
        ...
        <table class="table">
            <tbody>
                ...
                <tr>
                    ...
                    <td> {{ Tool::format_price($plan->price, $plan->currency->format) }} </td>
                </tr>
                ...
                <tr>
                    <td>{{ __('locale.plans.max_contact_groups') }}</td>
                ...
                <tr>
                    <td>{{ __('locale.plans.max_contacts_per_group') }}</td>
                ...
                <tr>
                    <td>{{ __('locale.plans.media_image_video_audio_messages') }}</td>
                    {{-- <td>{{ $plan->getOption('voice_sms') }} {{__('locale.labels.credit_per_sms')}}</td> --}}
                    <td>{{ $plan->displayMaxList() }}</td>
                </tr>
                <tr>
                    <td>{{ __('locale.plans.file_documnet_messages') }}</td>
                    {{-- <td>{{ $plan->getOption('voice_sms') }} {{__('locale.labels.credit_per_sms')}}</td> --}}
                    <td>{{ $plan->displayMaxList() }}</td>
                </tr>

                <tr>
                    <td>{{ __('locale.plans.contact_messages') }}</td>
        ...
        <div class="row">
            ...
            <div class="col-12">
                <ul class="nav nav-pills mb-2 mt-3 text-uppercase" role="tablist">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" id="account-tab" data-bs-toggle="tab" href="#logs"
                            aria-controls="logs" role="tab" aria-selected="true">
                            <i data-feather="database" class="mr-25"></i>{{ __('locale.subscription.logs') }}
                        </a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link" id="information-tab" data-bs-toggle="tab" href="#transactions"
                            aria-controls="transactions" role="tab" aria-selected="false">
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    @forelse ($subscription->getLogs() as $key => $log)
                                        <tr>
                                            <td> {{ Tool::customerDateTime($log->created_at) }} </td>
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    @forelse ($subscription->getTransactions() as $key => $invoice)
                                        <tr>
                                            <td>{{ Tool::customerDateTime($invoice->created_at) }}</td>
                                            <td>
                                                {!! $invoice->title !!}
