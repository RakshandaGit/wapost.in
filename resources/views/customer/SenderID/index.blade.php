    ...
    <script>
        $(document).ready(function () {
            ...
            let dataListView = $('.datatables-basic').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{{ route('customer.senderid.search') }}",
                    "dataType": "json",
                    "type": "POST",
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, full) {
                            let $actions = '';
                            if (full['can_delete']) {
                                $actions += '<span class="action-delete text-danger cursor-pointer me-1" data-bs-toggle="tooltip" data-placement="top" title=' + full['delete'] + ' data-id=' + full['uid'] + '>' +
                ...
                responsive: {
                    details: {
                        display: $.fn.dataTable.Responsive.display.modal({
                            header: function (row) {
                                let data = row.data();
                                return 'Details of ' + data['sender_id'];
                            }
                        }),
                        type: 'column',
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        if (sender_ids.length > 0) {
                            ...
                            $.ajax({
                                url: "{{ route('customer.senderid.batch_action') }}",
                                type: "POST",
                                data: {
                                    _token: "{{csrf_token()}}",
                                    ids: sender_ids
