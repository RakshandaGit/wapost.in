...
@section('title', __('locale.menu.Blogs'))
    ...
    <section id="datatables-basic">
        <div class="mb-3 mt-2">
            ...
            @can('create administrator')
                <div class="btn-group">
                    <a href="{{route('admin.blogs.create')}}" class="btn btn-success waves-light waves-effect fw-bold mx-1"> {{__('locale.buttons.create')}} <i data-feather="plus-circle"></i></a>
                </div>
            @endcan
    ...
    <script>
        $(document).ready(function () {
            ...
            Table.delegate(".get_status", "click", function () {
                ...
                $.ajax({
                    url: "{{ url(config('app.admin_path').'/blogs')}}" + '/' + role_id + '/active',
                    type: "POST",
                    data: {
                        _token: "{{csrf_token()}}"
                    },
            ...
            Table.delegate(".action-delete", "click", function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url(config('app.admin_path').'/blogs')}}" + '/' + id,
                            type: "POST",
                            data: {
                                _method: 'DELETE',
                                _token: "{{csrf_token()}}"
            ...
            $(".bulk-enable").on('click', function (e) {
                ...
                Swal.fire({
                    title: "{{__('locale.labels.are_you_sure')}}",
                    text: "{{__('locale.administrator.enable_administrators')}}",
                    icon: 'warning',
                    showCancelButton: true,
                    ...
                    if (result.value) {
                        ...
                        if (blog_ids.length > 0) {
                            ...
                            $.ajax({
                                url: "{{ route('admin.blogs.batch_action') }}",
                                type: "POST",
                                data: {
                                    _token: "{{csrf_token()}}",
                                    action: 'enable',
            ...
            $(".bulk-disable").on('click', function (e) {
                ...
                Swal.fire({
                    title: "{{__('locale.labels.are_you_sure')}}",
                    text: "{{__('locale.administrator.disable_administrators')}}",
                    icon: 'warning',
                    showCancelButton: true,
            ...
            $(".bulk-delete").on('click', function (e) {
                ...
                Swal.fire({
                    ...
                    title: "{{__('locale.labels.are_you_sure')}}",
                    text: "{{__('locale.administrator.delete_administrators')}}",
                    icon: 'warning',
                    showCancelButton: true,
