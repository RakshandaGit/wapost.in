...
@section('title', __('locale.menu.Templates'))
    ...
    <section id="datatables-basic">
        <div class="mb-3 mt-2">
            ...
            @can('create templates')
                <div class="btn-group">
                    <a href="{{route('admin.templates.create')}}" class="btn btn-success waves-light waves-effect fw-bold mx-1"> {{__('locale.buttons.add_new')}} <i data-feather="plus-circle"></i></a>
                </div>
            @endcan
    ...
    <script>
        $(document).ready(function () {
            ...
            let dataListView = $('.datatables-basic').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{{ route('admin.templates.search') }}",
                    "dataType": "json",
                    "type": "POST",
                    ...
                    },
                    {
                        ...
                        render: function (data, type, full) {
                            return (
                                ...
                                '</a>' +
                                '<span class="action-delete text-danger cursor-pointer" data-id=' + full['uid'] + '>' +
            ...
            Table.delegate(".get_status", "click", function () {
                ...
                $.ajax({
                    url: "{{ url(config('app.admin_path').'/templates')}}" + '/' + template_id + '/active',
                    type: "POST",
                    data: {
                        _token: "{{csrf_token()}}"
                    },
            ...
            Table.delegate(".action-delete", "click", function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url(config('app.admin_path').'/templates')}}" + '/' + id,
                            type: "POST",
                            data: {
                                _method: 'DELETE',
                                _token: "{{csrf_token()}}"
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $.each(rows_selected, function (index, rowId) {
                            template_ids.push(rowId)
                        });

                        if (template_ids.length > 0) {

                            $.ajax({
                                url: "{{ route('admin.templates.batch_action') }}",
                                type: "POST",
                                data: {
                                    _token: "{{csrf_token()}}",
                                    action: 'enable',
