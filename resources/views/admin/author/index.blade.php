    ...
    <section id="datatables-basic">
        <div class="mb-3 mt-2">
            ...
            @can('create administrator')
                <div class="btn-group">
                    <a href="{{route('admin.author.create')}}" class="btn btn-success waves-light waves-effect fw-bold mx-1"> {{__('locale.buttons.create')}} <i data-feather="plus-circle"></i></a>
                </div>
            @endcan
        ...
        $(document).ready(function () {
            ...
            Table.delegate(".action-delete", "click", function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url(config('app.admin_path').'/author')}}" + '/' + id,
                            type: "POST",
                            data: {
                                _method: 'DELETE',
                                _token: "{{csrf_token()}}"
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        if (blog_ids.length > 0) {
                            ...
                            $.ajax({
                                url: "{{ route('admin.author.batch_action') }}",
                                type: "POST",
                                data: {
                                    _token: "{{csrf_token()}}",
                                    action: 'destroy',
