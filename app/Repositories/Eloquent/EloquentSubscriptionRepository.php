...
use App\Library\PhonePe;
use App\Library\CashFree;
...
class EloquentSubscriptionRepository extends EloquentBaseRepository implements SubscriptionRepository
{
    ...
    public function payPayment(Plan $plan, Subscription $subscription, array $input): JsonResponse
    {
        ...
        if (isset($input['formType']) && $input['formType'] == 'addons') {
            $totalAddonsPrice = $input['total_addons'] * $plan->connection_addons_price;
            $totalAmount = $totalAddonsPrice;

            $currentSubscription = Auth::user()->customer->subscription ?? null;
            $currentPlanInvoice = Auth::user()->customer->subscription?->invoice ?? null;

            $input['total_frequency_duration'] = $currentPlanInvoice->duration_count;

            if ($currentSubscription && $currentPlanInvoice) {
                $isAddons = request()->segment(3) == 'addons';
                ...
                $lastTotalPerDayAddonsAmount = ($lastTotalAddonsAmount / $currentPlanInvoice->duration_count) / $totalDays;
                // $adjustedAddonsAmountForPerDayPerAddons = ($lastTotalPerDayAddonsAmount * $remainDays) / $currentPlanInvoice->addons_connections;
                if ($plan->frequency_unit == 'month' && $isAddons) {
                    $adjustedAddonsAmountForPerDayPerAddons = number_format(($currentPlanInvoice->connection_addons_price / 30) * $remainDays, 2, '.', '');
                } else if ($plan->frequency_unit == 'year' && $isAddons) {
                    $adjustedAddonsAmountForPerDayPerAddons = number_format(($currentPlanInvoice->connection_addons_price / 365) * $remainDays, 2, '.', '');
                }

                $totalAmount = number_format($adjustedAddonsAmountForPerDayPerAddons * $input['total_addons'], 2, '.', '');
                Session::put('isAddons',  $isAddons);
            }
        } else {
            $totalAddonsPrice = ($input['total_addons'] * $plan->connection_addons_price) * $input['total_frequency_duration'];
            $totalPlanPrice = $plan->price * $input['total_frequency_duration'];
            ...
            if ($currentSubscription && $currentPlanInvoice) {
                ...
                $lastTotalPerDayPlanAmount = $lastTotalPlanAmount / $totalDays;
                $lastTotalPerDayAddonsAmount = $lastTotalAddonsAmount / $totalDays;
                $isPlanNotExpired = $currentSubscription->current_period_ends_at > date('Y-m-d H:i:s');
                $isRenw = request()->segment(3) == 'renew';
                if ($isPlanNotExpired && !$isRenw) {
                    $adjustedPlanAmount = $lastTotalPerDayPlanAmount * $remainDays;
                    $adjustedAddonsAmount = $lastTotalPerDayAddonsAmount * $remainDays;
                }

                $totalAdjustedAmount = (float)$adjustedPlanAmount + $adjustedAddonsAmount;
                $totalAmount = number_format($totalAmount - $totalAdjustedAmount, 2, '.', '');
                ...
                if ($currentSubscription->addons_connections > $input['total_addons']) {
                    $adjAddonsPrice = ($currentSubscription->addons_connections - $input['total_addons']) * $calculatedPrice;
                    $totalAmount = $totalAmount - $adjAddonsPrice;
                } else {
                    $adjAddonsPrice = ($input['total_addons'] - $currentSubscription->addons_connections) * $calculatedPrice;
                    $totalAmount = $totalAmount + $adjAddonsPrice;
                }
                Session::put('isRenw',  $isRenw);
            }
        }
            ...
            switch ($paymentMethod->type) {
                ...
                case PaymentMethods::TYPE_STRIPE:
                    ...
                    try {
                        ...
                        Session::put('total_addons', $input['total_addons']);
                        Session::put('total_frequency_duration', $input['total_frequency_duration']);
                        return response()->json([
                            'status'          => 'success',
                ...
                case PaymentMethods::TYPE_PHONEPE:
                    ...
                    $phonepe = new PhonePe($environment);
                    $user = auth()->user();
                    $data['credentials'] = $credentials;
                    $data['plan'] = $plan;
                    $data['totalAddons'] = $input['total_addons'];
                    $data['totalFrequencyDuration'] = $input['total_frequency_duration'];
                    $data['user'] = $user;
                    $data['paymentMethod'] = $paymentMethod;
                    $requestJson = $phonepe->getRequestJson($data, $totalAmount);
                    $b64 = base64_encode($requestJson);
                    $sha256 = hash('sha256', $b64 . "/pg/v1/pay" . $credentials->merchant_salt) . "###1";
                   
                    $client = new \GuzzleHttp\Client();
                    $response = $client->request('POST', $phonepe->_url, [
                        'body' => '{"request":"' . $b64 . '"}',
                        'headers' => [
                            'X-VERIFY' => $sha256,
                            'Content-Type' => 'application/json',
                    ...
                    if ($result['success']) {
                        Session::put('phonpe_merchant_transaction_id', $result['data']['merchantTransactionId']);

                        $plan->update([
                            'transaction_id' => $result['data']['merchantTransactionId'],
                        ]);
                        if ($result['data']['instrumentResponse']['type'] == 'PAY_PAGE') {
                            return response()->json([
                                'status'       => 'success',
                    ...
                    exit();
                case PaymentMethods::TYPE_CASHFREE:

                    Session::put('payment_method', $paymentMethod->type);
                    $environment = $credentials->environment;
                    $payumoney = new CashFree($environment);
                    $user = auth()->user();
                    $order_id = "ORD126";
                    $body = '{"customer_details":{"customer_id":"7112AAA812234","customer_email":"john@cashfree.com","customer_phone":"9908734801"},"order_meta":{"notify_url":"'.route('user.registers.payment_success', ['user' => $user->uid, 'plan' => $plan->uid, 'payment_method' => $paymentMethod->uid]).'"},"order_id":"'.$order_id.'","order_amount":20,"order_currency":"INR"}';
                    // $body = '{"customer_details":{"customer_id":"7112AAA812234","customer_email":"john@cashfree.com","customer_phone":"9908734801"},"order_meta":{"notify_url":"' . route('registers.payment_success', ['user' => $user->uid, 'plan' => $plan->uid, 'payment_method' => $paymentMethod->uid]) . '"},"order_id":"' . $order_id . '","order_amount":20,"order_currency":"INR"}';
                    // $body = '{"customer_details":{"customer_id":"7112AAA812234","customer_email":"john@cashfree.com","customer_phone":"9908734801"},"order_meta":{"return_url":"'.route('user.registers.payment_success', ['user' => $user->uid, 'plan' => $plan->uid, 'payment_method' => $paymentMethod->uid]).'?order_id={order_id}","notify_url":"'.route('user.registers.payment_success', ['user' => $user->uid, 'plan' => $plan->uid, 'payment_method' => $paymentMethod->uid]).'"},"order_id":"ww123","order_amount":20,"order_currency":"INR"}';

                    // Response
                    // '{
                    //     "cf_order_id": 4830607,
                    //     "created_at": "2023-04-07T18:12:46+05:30",
                    //     "customer_details": {
                    ...
                    //     "order_amount": 20,
                    //     "order_currency": "INR",
                    //     "order_expiry_time": "2023-05-07T18:12:46+05:30",
                    //     "order_id": "ww124",
                    //     "order_meta": {
                    ...
                    //     "order_status": "ACTIVE",
                    //     "order_tags": null,
                    //     "payment_session_id": "session_R3vPY5nVIkvKulsSnBuNlf3rh--ZndM1X1dMh1vbKGw5rWK-mLzZJYHRFYU-c9EVA82eRn0Mmu1U1xQYrWww12PUnANRtEp9Tdcl5e45hyRJ",
                    //     "payments": {
                    //       "url": "https://sandbox.cashfree.com/pg/orders/ww124/payments"
                    //     },
                    //     "refunds": {
                    //       "url": "https://sandbox.cashfree.com/pg/orders/ww124/refunds"
                    //     },
                    //     "settlements": {
                    //       "url": "https://sandbox.cashfree.com/pg/orders/ww124/settlements"
                    //     },
                    //     "terminal_data": null
                    //   }';
                    ...
                    $response = $client->request('POST', 'https://sandbox.cashfree.com/pg/orders', [
                        ...
                        'headers' => [
                            ...
                            'x-client-id' => 'TEST3606839ae3a1d8cd8d4ced4559386063',
                            'x-client-secret' => 'TESTa6e2b779787ed60de64ddf590dfb80d4876cacac',
                        ],
                    ]);
                    ...
                    $response = $client->request('POST', 'https://sandbox.cashfree.com/pg/orders/sessions', [
                        'body' => '{"payment_method":{"upi":{"upi_id":"testsuccess@gocash","upi_expiry_minutes":10,"channel":"link"}},"payment_session_id":"' . $result['payment_session_id'] . '"}',
                        'headers' => [
                            'accept' => 'application/json',
                            'content-type' => 'application/json',
                            'x-api-version' => '2022-09-01',
                    ...
                    return response()->json([
                        'status'       => 'success',
                        'redirect_url' => $result['data']['payload']['phonepe'],
                    ]);
                    if ($result['success']) {
                        Session::put('phonpe_merchant_transaction_id', $result['data']['merchantTransactionId']);
        ...
        if ($paymentMethod) {
            ...
            switch ($paymentMethod->type) {
                ...
                case PaymentMethods::TYPE_PAYPAL:
                    ...
                    $request->body = [
                        ...
                        "application_context" => [
                            ...
                            // "return_url" => route('registers.payment_success', ['user' => $user->uid, 'plan' => $plan->uid, 'payment_method' => $paymentMethod->uid]),
                        ],
                    ];
                ...
                case PaymentMethods::TYPE_STRIPE:
                    ...
                    try {
                        $checkout_session = \Stripe\Checkout\Session::create([
                            ...
                            // 'success_url'          => route('registers.payment_success', ['user' => $user->uid, 'plan' => $plan->uid, 'payment_method' => $paymentMethod->uid]),
                        ]);
                ...
                case PaymentMethods::TYPE_2CHECKOUT:
                    ...
                    // $checkout->param('return_url', route('registers.payment_success', ['user' => $user->uid, 'plan' => $plan->uid, 'payment_method' => $paymentMethod->uid]));
                    $checkout->param('li_0_name', $item_name);
                    $checkout->param('li_0_price', $plan->price);
                    $checkout->param('li_0_quantity', 1);
                    $checkout->param('card_holder_name', $input['first_name'] . ' ' . $input['last_name']);
                ...
                case PaymentMethods::TYPE_PAYU:
                    ...
                    // $payu->param('confirmationUrl', route('registers.payment_success', ['user' => $user->uid, 'plan' => $plan->uid, 'payment_method' => $paymentMethod->uid]));
                ...
                case PaymentMethods::TYPE_INSTAMOJO:
                    ...
                    $payload = [
                        ...
                        // 'redirect_url'            => route('registers.payment_success', ['user' => $user->uid, 'plan' => $plan->uid, 'payment_method' => $paymentMethod->uid]),
                        'send_email'              => true,
                        'email'                   => $input['email'],
                        'allow_repeated_payments' => false,
                    ];
                ...
                case PaymentMethods::TYPE_PAYUMONEY:
                    ...
                    // $payumoney->param('surl', route('registers.payment_success', ['user' => $user->uid, 'plan' => $plan->uid, 'payment_method' => $paymentMethod->uid]));
                ...
                case PaymentMethods::TYPE_PHONEPE:
                    ...
                    $data['user'] = $user;
                    $data['paymentMethod'] = $paymentMethod;
                    $requestJson = $phonepe->getRequestJson($data);
                    $b64 = base64_encode($requestJson);
                    $sha256 = hash('sha256', $b64 . "/pg/v1/pay" . $credentials->merchant_salt) . "###1";
                    $client = new \GuzzleHttp\Client();
                    $response = $client->request('POST', $phonepe->_url, [
                ...
                case PaymentMethods::TYPE_PAYGATEGLOBAL:
                    ...
                    $parameters = [
                        ...
                        // 'url'      => route('registers.payment_success', ['user' => $user->uid, 'plan' => $plan->uid, 'payment_method' => $paymentMethod->uid]),
                    ];
                    $parameters = http_build_query($parameters);
                ...
                case PaymentMethods::TYPE_ORANGEMONEY:
                    ...
                    $data = [
                        ...
                        // "notif_url"    => route('registers.payment_success', ['user' => $user->uid, 'plan' => $plan->uid, 'payment_method' => $paymentMethod->uid]),
                        "lang"         => config('app.locale'),
                        "reference"    => $plan->uid,
                    ];
