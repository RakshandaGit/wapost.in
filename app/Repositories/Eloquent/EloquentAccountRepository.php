...
class EloquentAccountRepository extends EloquentBaseRepository implements AccountRepository
{
    ...
    public function register(array $input): User
    {
        ...
        if ($customer) {
            ...
            $customer->company  = $input['company'] ?? null;
        ...
        ApiDetail::create([
            'username' => 'WA' . $randomNumber,
            'password' => Str::random(8),
            'sendername' => Str::upper(Str::random(8)),
            'routetype' => 1,
            'user_id' => $user->id
        ]);
        ...
        if (isset($input['is_login_require']) && $input['is_login_require'] != false) {
...
POSTXML;
                ...
                case PaymentMethods::TYPE_PAYHERELK:
                    ...
                    Session::put('payment_method', $paymentMethod->type);
                    $order_id = str_random(10);
                    $md5sig   = strtoupper(md5($credentials->merchant_id . $order_id . $price . $currency_code . strtoupper(md5($credentials->merchant_secret))));
                    ...
                    $checkout->param('return_url', route('customer.callback.payherelk.top_up', ['user_id' => auth()->user()->id, 'sms_unit' => $input['sms_unit']]));
                    $checkout->param('cancel_url', route('customer.callback.payherelk.top_up'));
                    $checkout->param('notify_url', route('customer.callback.payherelk.top_up'));
