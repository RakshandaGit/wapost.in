...
class LoginController extends Controller
{
    ...
    */
    ...
    protected array $supportedProviders = ['facebook', 'google', 'github', 'twitter'];
    ...
    {
        $this->middleware('guest')->except('logout', 'avatar', 'downloadSampleFile', 'downloadUserSampleFile');
    ...
    public function login(Request $request): RedirectResponse
    {
        ...
        try {
            // $credentials = request(['username', 'password', 'status' => 1]);
            ...
            if (preg_match('/^[0-9]+$/', $request->username) && $request->password != null) {
                if (Helper::formatMobileNumber($request->username)) {
                    $request->username = Helper::formatMobileNumber($request->username);
                }
            }

            $checkUser = User::where('phone', $request->username)
            ...
            if (config('app.stage' != 'demo')) {
                if (Helper::app_config('login_notification_email') && $user->is_admin) {
                    $user->notify(new LoginConfirmation());
                }

                if ($user->is_customer && $user->customer->getNotifications()['login'] == 'yes') {
                    $user->notify(new LoginConfirmation());
                }
            }
