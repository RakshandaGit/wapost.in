...
class AccountController extends Controller
{
         ...
         */
        public function changePassword(Request $request): Redirector|Application|RedirectResponse
        {
                if (config('app.stage') == 'demo') {
                ...
                $rules = [
                        ...
                        'password_confirmation' => 'required|min:8|required_with:password_confirmation|same:password_confirmation',

                ];
                $stab = Validator::make($input, $rules);
                if ($stab->fails()) {
                        return back()->withInput(['tab' => 'security'])->withErrors($stab->errors());
                }
                Auth::user()->update([
        ...
        public function updateInformation(Request $request): RedirectResponse
        {
                ...
                $input = $request->except('_token');
                $rules = [
                        'phone'      => ['required', new Phone($request->phone), 'numeric', 'digits:12', 'unique:users,phone,' . Auth::id()],
                ...
                if ($v->fails()) {
                        return back()->withInput(['tab' => 'information'])->withErrors($v->errors());
                }
    ...
    */
        ...
        public function successfulRegisterPayment(User $user, Plan $plan, PaymentMethods $payment_method, Request $request): RedirectResponse
        {
                ...
                if ($currentSubscription && $currentPlanInvoice) {
                        $input['total_frequency_duration'] = $currentPlanInvoice->duration_count; //need to check
                ...
                switch ($payment_method->type) {
                                        ...
                                        try {
                                                ...
                                                if ($response->payment_status == 'paid') {
                                                        ...
                                                        if ($invoice) {
                                                                ...
                                                                $subscription->plan_id                = $plan->getBillableId();
                                                                $subscription->end_period_last_days   = '10';
                                                                $subscription->current_period_ends_at = $subscription->getPeriodEndsAt(Carbon::now(), $totalFrequencyDuration);
                                                                $subscription->end_at                 = null;
                                                                $subscription->end_by                 = null;
                                                                $subscription->payment_method_id      = $paymentMethod->id;
                                                                ...
                                                                // sending email welcome and subscription
                                                                $this->sendWelcomeEmail($user);
                                                                ...
                                                                $subCLog = Subscription::where('user_id', $user->id)->with('subscriptionLogs')->first();
                                                                Auth::login($user, true); //added by shubham malewar
                                                                if ($subCLog->subscriptionLogs->count() > 1) {
                                                                        $SuccessRoute = 'user.home';
                                                                        return redirect()->route($SuccessRoute)->with([
                                                                                'status'  => 'success',
                                                                                'message' => __('locale.payment_gateways.payment_successfully_made'),
                                                                        ]);
                                                                } else {
                                                                        $SuccessRoute = 'thank-you?paymentStatus=PAYMENT_SUCCESS';
                                                                        return Redirect::to($SuccessRoute)->with([
                                                                                'status'  => 'success',
                                                                                'message' => __('locale.payment_gateways.payment_successfully_made'),
                                                                        ]);
                                                                }
                                                        ...
                                                        //         'message' => __('locale.exceptions.something_went_wrong'),
                                                        // ]);
                                                        return Redirect::to('thank-you?paymentStatus=PAYMENT_ERROR')->with([
                                                                'status'  => 'error',
                                                                'message' => __('locale.exceptions.something_went_wrong'),
                                                        ]);
                                                } else {
                                ...
                                if (isset($pollurl)) {
                                        ...
                                        if ($paymentMethod) {
                                                ...
                                                $paynow = new Paynow(
                                                        ...
                                                        // route('registers.payment_success', ['user' => $user->uid, 'plan' => $plan->uid, 'payment_method' => $paymentMethod->uid])
                                                );
                        ...
                        case PaymentMethods::TYPE_PHONEPE:
                                if ($request->code == 'PAYMENT_SUCCESS') {
                                        // if ($request->code == 'PAYMENT_ERROR' || $request->code == 'PAYMENT_PENDING') {
                                        $paymentMethod = PaymentMethods::where('status', true)->where('type', PaymentMethods::TYPE_PHONEPE)->first();
                                        if ($paymentMethod) {
                                                if ($request->transactionId) {
                                                        $invoice_exist = Invoices::where('transaction_id', $request->transactionId)->first();
                                                        ...
                                                        if (!$invoice_exist) {
                                                                ...
                                                                if ($invoice) {
                                                                        ...
                                                                        if ($isAddons) {
                                                                                ...
                                                                                // $subscription->end_at                 = null;
                                                                                if ($plan->frequency_unit == 'month' && $totalFrequencyDuration != 1) {
                                                                                        $subscription->end_at                 = Carbon::now()->addMonths($totalFrequencyDuration);
                                                                                } elseif ($plan->frequency_unit == 'year' && $totalFrequencyDuration != 1) {
                                                                                        $subscription->end_at                 = Carbon::now()->addYears($totalFrequencyDuration);
                                                                                }
                                                                                $subscription->end_by                 = null;
                                                                                $subscription->payment_method_id      = $paymentMethod->id;
                                                                                $subscription->invoice_id             = $invoice->id;
                                                                        ...
                                                                        Auth::login($user, true); //added by shubham malewar
                                                                        $request->session()->forget(['payment_method', 'total_frequency_duration', 'total_addons', 'isRenw', 'isAddons']);
                                                                        if ($subCLog->subscriptionLogs->count() > 1 || $isAddons) {
                                                                                $SuccessRoute = 'user.home';
                                                                                return redirect()->route($SuccessRoute)->with([
                                                                                        'status'  => 'success',
                                                                                        'message' => __('locale.payment_gateways.payment_successfully_made'),
                                                                                ...
                                                                                // sending welcome and subscription mail
                                                                                $this->sendWelcomeEmail($user);
        ...
        public function createUserKeys($data)
        {
        }
}
