...
class PlanController extends AdminBaseController
{
    ...
    #[NoReturn] public function search(Request $request): void
    {
        ...
        if (empty($request->input('search.value'))) {
            ...
            $plans = Plan::whereLike(['uid', 'name', 'price', 'connection_addons_price', 'frequency_unit', 'frequency_amount'], $search)
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();

            $totalFiltered = Plan::whereLike(['uid', 'name', 'price', 'connection_addons_price', 'frequency_unit', 'frequency_amount'], $search)->count();
        }
        ...
        if (!empty($plans)) {
            foreach ($plans as $plan) {
                ...
                $nestedData['addons']         = "<div>
                                                        <p class='fw-bold'>" . Tool::format_price($plan->connection_addons_price, $plan->currency->format) . " </p>
                                                   </div>";
    ...
    public function activeToggle(Plan $plan): JsonResponse
    {
        ...
        try {
            ...
            if ($plan->status == 0 && $plan->hasPrimarySendingServer() && $plan->hasPricingCoverage()) {
            ...
            } elseif ($plan->status == 1 && $plan->hasPrimarySendingServer() && $plan->hasPricingCoverage()) {
